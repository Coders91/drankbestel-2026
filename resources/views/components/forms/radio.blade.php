...
<div class="relative flex items-center justify-start gap-2.5">
  <input
    ...
    value="{{ $value }}"
    {{ $attributes->merge([
        'class' => "flex-shrink-0 peer before:content[''] relative size-5 appearance-none rounded-full border border-gray-400 bg-white
  ...
  @if($label)
    {{ $label }}
  @else
    <label for="{{ $id }}" class="text-base cursor-pointer peer-disabled:cursor-not-allowed flex justify-between items-center gap-2 w-full">
      {{ $slot }}
    </label>
  @endif
</div>
