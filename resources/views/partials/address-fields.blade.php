<x-field-group class="flex gap-4">
  <div class="flex-1">
    <x-form-field id="{{ $type }}_postcode"
                  name="{{ $type }}_postcode"
                  wire:model.live.blur="form.{{ $type }}_postcode"
                  placeholder="1234AB"
  ...
  <div class="basis-1/3">
    <x-form-field id="{{ $type }}_house_number"
                  ...
                  wire:model.live.blur="form.{{ $type }}_house_number"
                  placeholder="123"
                  x-bind:disabled="{{ $type }}_loading"
                  @input="touched[$el.name] = true"
                  @blur="getAddressData('{{ $type }}', $el)"
  ...
  <div class="basis-1/4">
    <x-form-field id="{{ $type }}_house_number_suffix"
                  ...
                  wire:model.live.blur="form.{{ $type }}_house_number_suffix"
                  placeholder="A"
                  x-bind:disabled="{{ $type }}_loading"
                  @input="touched[$el.name] = true"
                  @blur="getAddressData('{{ $type }}', $el)"
...
<x-field-group class="flex gap-4">
  <div class="flex-1">
    <x-form-field readonly
                  ...
                  wire:model.live.blur="form.{{ $type }}_street_name"
                  placeholder="Straat"
                  @input="touched[$el.name] = true;"
  ...
  <div class="flex-1">
    <x-form-field readonly
                  ...
                  id="{{ $type }}_city"
                  name="{{ $type }}_city"
                  wire:model.live.blur="form.{{ $type }}_city"
                  placeholder="Plaats"
                  @input="touched[$el.name] = true"
                  @blur="validateField($el)"
    >
